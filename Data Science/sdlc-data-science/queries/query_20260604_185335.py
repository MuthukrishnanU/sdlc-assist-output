...
transactionsInfo = spark.read.format("parquet").load("/path/to/transactionsInfo")
...
upi_count = upi_transactions.groupBy("customer_id").agg(count("transaction_id").alias("upi_count"))

# Join UPI count with customer loans
customer_loans = customer_loans.join(upi_count, "customer_id", "left")

# Add UPI inclined flag
customer_loans = customer_loans.withColumn(
    "upi_inclined",
    when(col("upi_count") > 10, lit(True)).otherwise(lit(False))
)

# Join with transactionsInfo to get transaction details
final_df = customer_loans.join(transactionsInfo, "customer_id", "left")
